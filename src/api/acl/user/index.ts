...
export const reqAllUserInfo = (
  ...
  return http.get<any, UserResponseData>(
...
export const reqAllRoles = (userId: number) =>
  http.get<any, AllRolesResponseData>(`${API.ALLROLE_URl}${userId}`)
//给已有的用户分配角色接口
export const reqSetRole = (data: SetRoleData) =>
  http.post<any, any>(API.SETROLE_URL, data)
//删除某一个账号
export const reqDeleteUser = (id: number) =>
  http.delete<any, any>(`${API.DELETEUSER_URL}${id}`)
//批量删除接口
export const reqDeleteAllUser = (idList: number[]) =>
  http.delete<any, any>(API.DELETEALLUSER_URL, { data: idList })
