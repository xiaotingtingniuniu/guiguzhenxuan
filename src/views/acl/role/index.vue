<template>
  <el-card>
    <el-form inline class="form">
      <el-form-item label="职位搜索">
        <el-input placeholder="请输入搜索职位关键字" v-model="searchRoleName"></el-input>
      </el-form-item>
      <el-form-item>
  ...
  <el-card class="card-table">
    <el-button icon="Plus" type="primary" class="plus" @click="addRole">添加职位</el-button>
    <el-table :data="roleArr" border>
      ...
      <el-table-column label="操作" width="300" align="center">
        <template #="{ row, $index }">
          <el-button type="primary" size="small" icon="User" @click="setPermissions(row)">分配权限</el-button>
          <el-button type="primary" size="small" icon="Edit" @click="edit(row)">编辑</el-button>
          <el-popconfirm :title="`确定删除'${row.roleName}'吗？`" @confirm="deleteRole(row.id)" width="180">
            <template #reference>
    ...
    <el-pagination class="pagination" v-model:current-page="pageNo" v-model:page-size="pageSize"
      :page-sizes="[10, 20, 30, 40]" size="small" layout="prev, pager, next, jumper, -> ,total, sizes," :total="total"
  ...
  <el-drawer v-model="drawer">
    ...
    <template #default>
      <el-tree style="max-width: 600px" ref='tree' :data="menuAndButtonList" show-checkbox node-key="id"
...
import { reqAllPermission, reqDeleteRole, reqGetALLRole, reqSaveOrUpdateRole, reqSetPermission } from '../../../api/acl/role';
import { GetAllRoleResponseData, GetPermissionResponseData, MenuOrButtonList, Role } from '../../../api/acl/role/type';
//引入layout小仓库
...
const getRole = async (paper = 1) => {
  pageNo.value = paper;
  const result: GetAllRoleResponseData = await reqGetALLRole(pageNo.value, pageSize.value, searchRoleName.value);
