<template>
  <div>
    <el-card class="top">
      <el-form :inline="true" class="form">
        <el-form-item label="用户名：">
          <el-input placeholder="请你输入搜索用户名字" v-model="searchUserName"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" size="default" @click="seach" :disabled="searchUserName?false:true">搜索</el-button>
    ...
    <el-card class="bottom">
      ...
      <el-button type="primary" size="default" @click="deleteAllUser" :disabled="idList.length>0?false:true">批量删除</el-button>
      <!-- table 展示用户信息 -->
      <el-table class="table" :data="userArr" border style="width: 100%" @selection-change="selectChange">
        ...
        <el-table-column label="操作" width="300" align="center">
          <template #="{ row, $index }">
            ...
            <el-popconfirm :title="`确定删除账户'${row.username}'吗?`" @confirm="deleteUser(row.id)" width="260px">
              <template #reference>
      ...
      <!-- 分页器 -->
      <el-pagination v-model:current-page="pageNo" v-model:page-size="pageSize" :page-sizes="[5, 7, 9, 11]" size="small"
    ...
    <el-drawer v-model="drawer1">
      ...
      <template #default>
        <div>
          <el-form :model="userParams" :rules="rules1" ref="formRef1">
            <el-form-item label="用户姓名" prop="username">
              <el-input placeholder="请输入用户名字" v-model="userParams.username" disabled></el-input>
            </el-form-item>
...
import { reqAddUserOrUpdateUser, reqAllRoles, reqAllUserInfo, reqDeleteAllUser, reqDeleteUser, reqSetRole } from '../../../api/acl/user';
import { AllRolesResponseData, Records, RoleArr, SetRoleData, User, UserResponseData } from '../../../api/acl/user/type';
...
const getAllUserInfo = async (pager = 1) => {
  pageNo.value = pager;
  const result: UserResponseData = await reqAllUserInfo(pageNo.value, pageSize.value,searchUserName.value);
  if(result.code==200){
