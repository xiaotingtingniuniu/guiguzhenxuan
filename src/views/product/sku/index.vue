<template>
  <el-card style="width: 100%;">
      ...
      <el-table-column label="操作" width="400px" fixed="right">
        <template #="{ row, $index }">
          <el-button type="primary" size="small" :icon="row.isSale == 1 ? 'Bottom' : 'Top'"
            @click="saleOrCancelSale(row)"></el-button>
    ...
    <el-pagination v-model:current-page="pageNo" v-model:page-size="pageSize" :page-sizes="[10, 20, 30, 40]"
      size="small" layout="prev, pager, next, jumper,->, sizes, total," :total="total" @size-change="handleSizeChange"
    ...
    <el-drawer v-model="drawer">
      ...
      <template #default>
        ...
        <el-row style="margin:10px 0">
          ...
          <el-col :span="18">
            <el-tag v-for="item in skuInfor.skuAttrValueList" style="margin:5px" :key="item.id">{{ item.valueName
              }}</el-tag>
          </el-col>
        </el-row>
        <el-row style="margin:10px 0">
          ...
          <el-col :span="18">
            <el-tag v-for="item in skuInfor.skuSaleAttrValueList" style="margin:5px" :key="item.id">{{
        ...
        <el-row style="margin:10px 0">
          ...
          <el-col :span="18">
            <el-carousel :interval="4000" type="card" height="200px">
              <el-carousel-item v-for="item in skuInfor.skuImageList" :key="item.id">
...
import { reqCancelSale, reqDeleteSku, reqGetSkuList, reqOnSale, reqSkuInfor } from '../../../api/product/sku';
import { GetSkuListResponseData, SkuData, SkuInfor, SkuInforResponseData } from '../../../api/product/sku/type';
...
const getSkuData = async (page = 1) => {
  pageNo.value = page;
  const result: GetSkuListResponseData = await reqGetSkuList(pageNo.value, pageSize.value);
  if (result.code === 200) {
