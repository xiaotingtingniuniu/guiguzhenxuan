  ...
  <div>
    <el-form label-width="100px">
      ...
      <el-form-item label="SPU品牌">
        <el-select placeholder="请你选择品牌" style="width: 240px" v-model="spuParams.tmId">
          <el-option v-for="item in allTradeMark" :key="item.id" :label="item.tmName" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="SPU描述">
        <el-input type="textarea" placeholder="请你输入SPU的 描述" v-model='spuParams.description'></el-input>
      </el-form-item>
      ...
      <el-form-item label="SPU销售属性">
        <el-select :placeholder="unSelectSaleAttr.length ? `还未选择的有${unSelectSaleAttr.length}个` : '无'"
          style="width: 240px" v-model="saleAttrIdAndValueName">
          ...
          <el-table-column label="销售属性值">
            <template #="{ row, $index }">
              <div class="flex gap-2">
                <el-tag v-for="(tag, index) in row.spuSaleAttrValueList" :key="tag.id" closable type="primary"
                  style="margin:0 5px" @close="row.spuSaleAttrValueList.splice(index, 1)">
                  {{ tag.saleAttrValueName }}
                </el-tag>
                <el-input v-if="row.flag === true" ref="InputRef" v-model="row.saleAttrValue" class="w-20" size="small"
                  @keyup.enter="handleInputConfirm(row)" @blur="handleInputConfirm(row)" placeholder="请输入属性值" />
                <el-button v-else class="button-new-tag" type="primary" size="small" icon="Plus"
                  @click="showInput(row)"></el-button>
              </div>
            </template>
          </el-table-column>
      ...
      <el-form-item>
        <el-button type="primary" size="default" @click="save"
          :disabled="spuSaleAttrList.length > 0 ? false : true">保存</el-button>
...
import type { AllSaleAttr, AllSaleAttrListResponseData, Record, SpuHasImageListResponseData, SpuImageList, SpuSaleAttr, SpuSaleAttrListResponseData, SpuSaleAttrValue, Trademark, TrademarkListResponseData } from '../../../api/product/spu/type';
import { reqAddOrUpdateSpu, reqGetAllSaleAttrList, reqGetSpuSaleAttrList, reqGetTradeMarkImageList, reqGetTradeMarkList } from '../../../api/product/spu';
...
const getImageList = async (spuId: number) => {
  const result: SpuHasImageListResponseData = await reqGetTradeMarkImageList(spuId);
  console.log('result', result);
  if (result.code === 200) {
    //存储已有的spu照片墙
