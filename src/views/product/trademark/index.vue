<template>
  ...
  <el-card class="card">
    <!-- 卡片顶部添加品牌按钮 -->
    <el-button type="primary" size="primary" icon="Plus" @click="addTrademark">添加品牌</el-button>
    <!-- table表格组件 
         table
         -- border:可以设置表格纵向是有边框
         table-column
    ...
    <el-table border :data="trademarkArr" style="width: 100%" class="table">
      ...
      <el-table-column prop="updateTime" label="品牌操作">
        <template #="{ row, $index }">
          ...
          <!-- 气泡组件 -->
          <el-popconfirm :title="`您确定要删除${row.tmName}?`" @confirm="confirmEvent(row.id)" width="200px" icon="Delete">
            <template #reference>
              <el-button type="primary" size="small" icon="Delete"></el-button>
            </template>
          </el-popconfirm>
    ...
    -->
    <el-pagination v-model:current-page="pageNo" v-model:page-size="limit" :page-sizes="[3, 6, 9, 12]" size="small"
      layout="prev, pager, next, jumper,->,sizes, total" :total="total" @current-change="currentChange"
      @size-change="sizeChange" />
  </el-card>
  <!-- 对话框组件 -->
  <el-dialog v-model="dialogTableVisible" :title="form.id ? '修改品牌' : '添加品牌'" width="500" class="dialog">
    <el-form :model="form" :rules="rules" ref="formRef">
      <el-form-item label="品牌名称" class="form_width" label-width="90px" prop="tmName">
        <el-input autocomplete="off" placeholder="请输入品牌名称" v-model="form.tmName" />
      </el-form-item>
      <el-form-item label="品牌Logo" class="form_width" label-width="90px" prop="logoUrl">
...
import { HasTrademarkResponseData, Records, Record } from '../../../api/product/trademark/type';
...
const getTrademark = async (pager = 1) => {
  //不传参数时默认是访问第一页数据
  pageNo.value = pager;
  const result: HasTrademarkResponseData = await reqHasTrademark(pageNo.value, limit.value);
  if (result.code === 200) {
